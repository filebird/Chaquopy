package com.xiangxun.wane.test;
...
import com.xiangxun.wane.dao.JavaBean;

import java.util.ArrayList;
import java.util.List;

public class TestPython extends Activity {
    static final String TAG = "PythonOnAndroid";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_testpython);
        initPython();
        callPythonCode();
    }
    // 初始化Python环境
    ...
    void callPythonCode(){
        ...
        PyObject objmath = py.getModule("hello").callAttr("getsqrt", 9);
        Float datasqrt = objmath.toJava(Float.class);
        Log.d(TAG,"sqrt = "+ datasqrt.toString());

        PyObject copy = py.getModule("hello").callAttr("test_copy");
        Log.d(TAG,"copy = "+ copy.toString());

        PyObject deepcopy = py.getModule("hello").callAttr("test_deepcopy");
        Log.d(TAG,"deepcopy = "+ deepcopy.toString());
